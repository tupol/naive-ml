...
class KMeansGaussianSpec extends FunSuite with Matchers {
  ...
  test("KMeansGaussian#train test 2 discs") {
    ...
    val expectedProbabilities = Seq(0.9, 0.4, 0.001, 0.000001)
    ...
    def lpointToStr(lp: LabeledPoint) = f"(${lp._1}%.0f ${lp._2.map(x => f"$x%12.10f").mkString("[", ", ", "]")})"

    println("Clusters")
    kmeans.clusterCenters.map(lpointToStr).foreach(println)
    println("Predictions")
    points.foreach{ point =>
      println(f"${pointToStr(point)}  ${kmg.predict(point)}%12.10f  ${pointToStr(kmg.predictByDimension(point))}")
    }
  }
