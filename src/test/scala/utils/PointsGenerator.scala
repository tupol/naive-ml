...
/**
  * Generate cluster data in different shapes
  */
object PointsGenerator {

  type Coordinate = (Double, Double)

  def rectangle(points: Int = 200, origin: Coordinate = (-0.5, -0.5),
    ...
    (0 until points).
      map(x => (math.random * width + origin._1, math.random * height + origin._2))
  }

  def square(points: Int = 200, origin: Coordinate = (-0.5, -0.5), side: Double = 1): Seq[Coordinate] =
    rectangle(points, origin, side, side)

  def disc(points: Int = 200, center: Coordinate = (0, 0), radius: Double = 1): Seq[Coordinate] =
    ring(points, center, 0, radius)

  def ring(points: Int = 200, center: Coordinate = (0, 0), minRadius: Double = 0.5, maxRadius: Double = 1): Seq[Coordinate] =
    sector(points, center, minRadius, maxRadius, 0, 2 * math.Pi)

  def sector(points: Int = 200, center: Coordinate = (0, 0), minRadius: Double = 0.5, maxRadius: Double = 1,
    ...
    (0 until points).
      ...
      map{ case(r,t) =>
        (center._1 + r * cos(t), center._2 + r * sin(t))
      }
  }
