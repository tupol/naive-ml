...
class KMeansGuessKItTest extends FunSuite with Matchers {
  ...
  test("KMeans#guessK integration test for 9 clusters with a normal epsilon") {
    ...
    val bestModel = testGuessK(dataPoints, epsilon = NORMAL_SLOPE, expectedK = 9, expectedKTolerance = 2)
  ...
  test("KMeans#guessK integration test for 100 clusters with a normal epsilon.") {
    ...
    val originalCentroids = (0 until 10).flatMap(x => (0 until 10).map(y => Array(2.0 * x, 2.0 * y)))

    val dataPoints = originalCentroids.flatMap(p => disc(CLUSTER_SIZE, p, 0.25))

    val bestModel = testGuessK(dataPoints, epsilon = NORMAL_SLOPE, expectedK = 100, expectedKTolerance = 40)
  ...
  private def testGuessK(dataPoints: Seq[Point], epsilon: Double, expectedK: Int, expectedKTolerance: Int, runs: Int = 9) = {
    ...
    val reportFile = f"./ExperimentWithK_$expectedK%03d.txt"
    ...
    saveLinesToFile(Seq(s"Expected K, $expectedK, Data Size,${(0.7 * dataPoints.size).toInt}", ""), reportFile)

    val guessedK = KMeans.guessK(shuffled, runs, kMeansTrainerFactory, epsilon)(reportFile)
