...
object statsops {
  ...
  implicit def indexedSeq2Stats(value: IndexedSeq[Double]): Stats[IndexedSeq[Double]] = Stats.fromVectors(value)
  ...
  implicit object DoubleStatsOps extends StatsOps[Double] {
    ...
    def append(x: Stats[Double], y: Stats[Double]): Stats[Double] = {
      ...
      val sse = x.sse + y.sse + delta * delta * x.count * y.count / newCount // See https://en.wikipedia.org/wiki/Algorithms_for_calculating_variance#Parallel_algorithm
  ...
  implicit object ArrayStatsOps extends StatsOps[Point] {
    ...
    def append(x: Stats[Point], y: Stats[Point]): Stats[Point] = {
      ...
      val newAvg = x.avg.zip(y.avg).map { case (a, b) => (x.count * a + y.count * b) / newCount } // (x.count * x.avg + y.count * y.avg) / newCount
