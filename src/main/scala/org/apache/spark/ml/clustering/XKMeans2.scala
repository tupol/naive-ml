...
import org.apache.spark.ml.clustering.evaluation.{ClusteringDistanceMetrics, ClusteringStatisticalSummary}
...
class XKMeans2(override val uid: String) extends Estimator[XKMeans2Model] with XKMeans2Params {
  ...
  override def fit(dataset: DataFrame): XKMeans2Model = {
    ...
    val statisticalSummary = new ClusteringStatisticalSummary(rdd.map(x => (x._1, x._2)))

    val distanceMetrics = new ClusteringDistanceMetrics(rdd.map(x => (x._1, x._3)))

    val model = new XKMeans2Model(uid, distanceMetrics, statisticalSummary)
...
class XKMeans2Model private[ml](
                                ...
                                val statisticalSummary: ClusteringStatisticalSummary)
  extends Model[XKMeans2Model] with XKMeans2Params {

  override def copy(extra: ParamMap): XKMeans2Model = {
    val copied = new XKMeans2Model(uid, distanceMetrics, statisticalSummary)
  ...
  override def transform(dataset: DataFrame): DataFrame = {
    ...
    val featuresNo = statisticalSummary.byCluster(0).variance.size

    val varByFeatures = (0 until featuresNo).map{ f =>
      (0 until K).map{k => statisticalSummary.byCluster(k).variance(f)}}
    ...
    {
      val averages = statisticalSummary.byCluster(k).mean
      val variances = statisticalSummary.byCluster(k).variance
